            ...
            <section id="content">
                <?php
                    ...
                    if ($_SESSION['UserType']=="Admin"){
                     ...
                     $query="SELECT * FROM LUUsers WHERE UserType='Customer'";
                     $result=mysqli_query($connection, $query);
                     //Shows all customer details
                     echo '<h1>Manage Customers</h1> <br><br>';
                     echo '<h3>All customer details:</h3> <br><br>';
                     ...
                     while($row=mysqli_fetch_assoc($result)){
                         echo '<tr><td>'.$row['UserFirstname'].'</td><td>'.$row['UserSurname'].'</td><td>'.$row['UserEmail'].'</td><td>'.$row['UserAddress'].'</td><td>'.$row['UserTel'].'</td><td><a href="editAllUserDetails.php?id='. $row['UserID'].'">Edit Details</a></td></tr>';
                     }
                     echo '</table>';
