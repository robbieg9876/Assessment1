<!DOCTYPE html>
<html>

<head>
    <link href="Stylesheet/stylesheet.css" rel="stylesheet" type="text/css" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css?family=Montserrat&display=swap" rel="stylesheet">
    <link rel="icon" href="images/leadsunited.ico" type="image/x-icon" />
    <title>Leads United</title>
</head>

<body>
    <div class="grid">
        <div class="logo"><img src="images/leadsunited.png" alt="Leads United Logo" /></div>
        <div class="navbar">
            <a href="home.php">Home</a></li>
            <a href="services.php">Services</a>
            <a href="team.php">Our Team</a>
            <a href="gallery.php">Gallery</a>
            <a href="contact.php" class="active">Contact Us</a>
            <?php 
                //Adds relevant tabs to navbar
                session_start();
                if($_SESSION['user']==true){
                    echo $_SESSION["txtFirstname"];
                    echo '<a href = "account.php">My Account</a>';
                    echo '<a href = "logout.php"><span>Logout</span></a>';
                }
                else
                {
                    echo'<a href="login.php"><span>Login/Register</span></a>';
                }
            ?>
        </div>
        <div class="contact-container">
            <section id="contact">
                <h1 class="contact-header">Get in touch</h1>
                <div class="contact-wrapper">
                    <form class="form-contact" role="form" method="post" action="contact.php">

                        <div class="text-box">
                            <input type="text" class="info" id="name" placeholder="Name" name="name">
                        </div>

                        <div class="text-box">
                            <input type="email" class="info" id="email" placeholder="Email" name="email">
                        </div>

                        <textarea class="info" rows="8" placeholder="Message" name="message"></textarea>

                        <button class="send-button" id="submit" type="submit" value="Send" name='submit'>
                            <div class="button">
                                <span class="send-text">Send</span>
                            </div>
                        </button>
                        <?php
                            if(isset($_POST['submit'])){
                                //Emails the message 
                                $to = "robbiegiles@hotmail.co.uk";
                                $email_from=$_POST['email'];
                                $email_body=$_POST['message'];
                                $headers = "From: $email_from \r\n";
                                mail($to,$email_subject,$email_body,$headers);
                                echo'Unfortunatley you are unable to sent emails on this server.';
                            }
                        ?>
                    </form>

                    <div class="info-container">
                        <div class="message">
                            <h3>If you need to get in touch just fill out the form or use one of the methods bellow</h3>
                        </div>
                        <hr>
                        <div class="contactInfo">
                            <li class="Info-item"><span class="contact-text place">Leeds | UK</span></li>
                            <li class="Info-item"><span class="contact-text phone">011394960688</span></li>
                            <li class="Info-item"><span class="contact-text email"><a href="mailto:leadsunited2020@gmail.com" title="Send me an email">leadsunited2020@gmail.com</a></span></li>
                            <hr>
                        </div>
                    </div>
                </div>
            </section>
        </div>
        <footer>
            <p>All Right Reserved Leads United</p>
            <a href="https://www.instagram.com/leadsunited2020/"><img alt="Instagram" src="data:image/png;base64," height="30" width="30"> </a>
            <a href="https://twitter.com/united_leads"><img alt="Twitter" src="data:image/png;base64," height="30" width="30"> </a>
            <a href="https://www.facebook.com/LeadsUnited2020/"><img alt="Facebook" src="data:image/png;base64," height="30" width="30"> </a>
        </footer>
    </div>
</body>

</html>